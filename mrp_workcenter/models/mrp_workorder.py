...
class MrpWorkorder(models.Model):
    ...
    production_bom_id = fields.Many2one(
        'mrp.bom', related='production_id.bom_id')
    ...
    qty_remaining = fields.Float(
        'Quantity To Be Produced', compute='_compute_qty_remaining', digits='Product Unit of Measure')

    qty_produced = fields.Float(
        'Quantity', default=0.0, readonly=True,
    ...
    date_planned_start = fields.Datetime(
        'Scheduled Start Date', compute='_compute_dates_planned',
        inverse='_set_dates_planned',
        states={'done': [('readonly', True)], 'cancel': [('readonly', True)]},
        store=True, copy=False)

    date_planned_finished = fields.Datetime(
        'Scheduled End Date', compute='_compute_dates_planned',
        inverse='_set_dates_planned',
        states={'done': [('readonly', True)], 'cancel': [('readonly', True)]},
        store=True, copy=False)
    ...
    working_user_ids = fields.Many2many(
        ...
        string='Working Users')

    last_working_user_id = fields.Many2many(
        'res.users', 'mrp_workorder_last_users_rel',
        'workorder_id', 'user_id',
        string='Last Users')

    costs_hour = fields.Float(
        string='Cost per hour', default=0.0, group_operator="avg")
    ...
    json_popover = fields.Char(
        'Popover Data JSON', compute='_compute_json_popover')

    show_json_popover = fields.Boolean(
        'Show Popover?', compute='_compute_json_popover')

    consumption = fields.Selection(
        related='production_id.consumption')

    qty_reported_from_previous_wo = fields.Float(
        'Carried Quantity', digits='Product Unit of Measure', copy=False,
    ...
    check_ids = fields.One2many('mrp.check', 'workorder_id', string='Checks')
    ...
    def _compute_json_popover(self):
        ...
        for wo in self:
            ...
            if wo.state in ('pending', 'waiting', 'ready'):
                ...
                prev_start = min(
                    [workorder.date_planned_start for workorder in previous_wos]) if previous_wos else False
                prev_finished = max(
                    [workorder.date_planned_finished for workorder in previous_wos]) if previous_wos else False
    ...
    def name_get(self):
        ...
        for wo in self:
            if len(wo.production_id.workorder_ids) == 1:
                ...
                res.append((wo.id, "%s - %s - %s - %s" % (wo.production_id.workorder_ids.ids.index(wo._origin.id) + 1,
    ...
    def _compute_scrap_move_count(self):
        data = self.env['stock.scrap']._read_group([('workorder_id', 'in', self.ids)], ['workorder_id'],
                                                   ['workorder_id'])
