...
class GeneralView(models.TransientModel):
    ...
    def _get_report_value(self, data):
        ...
        # Step 2: Assign grouped lines to each account
        for res in account_res:
            res['move_lines'] = lines_by_account.get(res['account_id'], [])
        debit_total = 0
    ...
    def get_accounts_line(self, account_id, title):
        ...
        sql = ('''
            ...
            LEFT JOIN res_partner p ON (l.partner_id=p.id)
            LEFT JOIN LATERAL (
                SELECT jsonb_object_keys(l.analytic_distribution)::INT AS keys
            ) anl ON true
            LEFT JOIN account_analytic_account an ON (anl.keys = an.id)                
            JOIN account_journal j ON (l.journal_id=j.id)
            JOIN account_account a ON (l.account_id = a.id)
            ''' + WHERE + new_final_filter + '''
            GROUP BY l.id, m.id, l.account_id, l.date, j.code, l.currency_id, 
                     l.amount_currency, l.ref, l.name, m.name, c.symbol, c.position, 
                     p.name, anl.keys
        ...
        # Execute and stream rows in batches to avoid MemoryError from dictfetchall()
        cr.execute(sql, params)
        cols = [d.name for d in cr.description]

        # Pre-create mapping for accounts to collect lines per account (avoids keeping an extra giant list)
        account_lines_map = {aid: [] for aid in accounts.ids}

        fetch_size = 5000  # adjust lower if you still hit memory limits (e.g. 1000)
        while True:
            rows = cr.fetchmany(fetch_size)
            if not rows:
                break
            for row in rows:
                row_dict = dict(zip(cols, row))
                acct_id = row_dict.get('account_id')
                # only store rows for accounts we care about (should be true due to WHERE)
                if acct_id in account_lines_map:
                    account_lines_map[acct_id].append(row_dict)

        # Build the final account_res list using per-account lines instead of a single huge list
        account_res = []
        for account in accounts:
            ...
            res['id'] = account.id
            # only assign the move lines that belong to this account
            res['move_lines'] = account_lines_map.get(account.id, [])
