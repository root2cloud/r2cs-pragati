...
class CouponExcelReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, coupons):
        ...
        sheet.set_column('D:D', 25)   # Company
        sheet.set_column('E:E', 20)   # Designation
        sheet.set_column('F:F', 12)   # No. of Persons
        sheet.set_column('G:G', 15)   # Coupon Value
        sheet.set_column('H:H', 15)   # Issue Date
        sheet.set_column('I:I', 15)   # Expiry Date
        sheet.set_column('J:J', 20)   # Salesperson
        sheet.set_column('K:K', 15)   # Status
        sheet.set_column('L:L', 20)   # Redeemed Date
        sheet.set_column('M:M', 20)   # Redeemed By
        sheet.set_column('N:N', 25)   # Redeem Designation

        # Write headers
        headers = [
            'Reference', 'Coupon Code', 'Customer', 'Company',
            'Coupon Issue Designation', 'No. of Persons', 'Coupon Value',
            'Issue Date', 'Expiry Date', 'Salesperson', 'Status',
            'Redeemed Date', 'Redeemed By', 'Redeem Designation'
        ]
        ...
        for coupon in coupons:
            ...
            if redeem:
                ...
                redeem_designation = redeem.designation or ''
                total_redeemed += 1
            ...
            sheet.write(row, 3, coupon.company_name or '', border_format)  # Company Name
            sheet.write(row, 4, coupon.designation or '', border_format)  # Issue Designation
            sheet.write(row, 5, no_of_persons, border_format)
            sheet.write(row, 6, coupon.coupon_value, currency_format)
            sheet.write(row, 7, coupon.issue_date or '', date_format)
            sheet.write(row, 8, coupon.expiry_date or '', date_format)
            sheet.write(row, 9, coupon.salespersons_name or '', border_format)
            sheet.write(row, 10, coupon.state, border_format)
            sheet.write(row, 11, redeemed_date, date_format)
            sheet.write(row, 12, redeemed_by, border_format)
            sheet.write(row, 13, redeem_designation, border_format)  # Redeem Designation
